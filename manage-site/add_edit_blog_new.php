...
if (isset($_POST["sub"])) {
    ...
    if ($page_title <> "" && $status <> "" && $parent <> "" && $page_alias <> "") {
        ...
        if ($page_id <> "") {
            ...
            $sql = "INSERT INTO " . TABLE_PAGES . " (`org_id`, `page_title`, `page_desc`, `meta_keywords`, `meta_desc`, `parent`, `sort_order`, `status`, `page_alias`) VALUES 
				(:oid,:pt, :pdsc, :mkey, :mdesc, :parent, :so, :status, :palias)";
    ...
    <form method="post" action="" name="pages">
        ...
        <table id="tableForm">
            ...
            <tr>
                ...
                <td>
                    <textarea name="blog_data" id="page_desc"><?php echo stripslashes($details[0]["page_desc"]); ?></textarea>
                </td>
            </tr>
            <tr>
